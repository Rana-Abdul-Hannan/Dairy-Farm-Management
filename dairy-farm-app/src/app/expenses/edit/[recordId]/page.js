// src/app/expenses/edit/[recordId]/page.js
...
export default function EditExpensePage() {
  ...
  // State to hold the expense record's data for the form
  const [expenseRecordData, setExpenseRecordData] = useState({
    date: '',
    description: '',
  ...
  const [loadingExpense, setLoadingExpense] = useState(true); // Loading state for fetching expense data
  const [savingExpense, setSavingExpense] = useState(false); // Loading state for saving expense data
  ...
  // Function to fetch the specific expense record data from the API
  const fetchExpenseRecordData = async (id) => {
      setLoadingExpense(true); // Start loading state for fetching
      setError(null); // Clear previous errors

      try {
        // --- API Call to fetch the specific expense record data by ID ---
        const response = await fetch(`/api/expenses/${id}`); // GET request to dynamic API route

        if (!response.ok) {
          // If the response is not OK, throw an error
          const errorData = await response.json(); // Try to read error message from response body
          throw new Error(errorData.message || `Failed to fetch expense record with ID ${id}`);
        }

        // If the response is OK, parse the JSON data
        const data = await response.json();

        // Set the state with the fetched expense record data
        // Ensure all expected fields are present
         setExpenseRecordData({
            date: data.date || '',
        ...
        setLoadingExpense(false); // Data loaded, stop loading state

      } catch (err) {
        console.error("Error fetching expense record data:", err);
        setError(err.message || 'An unexpected error occurred while fetching data.'); // Set error state
        setLoadingExpense(false); // Stop loading state on error
      }
  };
  ...
  // useEffect to load the expense record and animals data when the page component mounts
  useEffect(() => {
    // Fetch expense record data only if recordId is available
    if (recordId) {
      fetchExpenseRecordData(recordId);
    } else {
       setError("Expense record ID not provided.");
  ...
  const handleInputChange = (event) => {
    ...
    setExpenseRecordData({
      ...expenseRecordData, // Spread existing data
  ...
  const handleSubmit = async (event) => { // Made function async
    ...
    if (!expenseRecordData.date || !expenseRecordData.description || !expenseRecordData.amount || !expenseRecordData.category) {
    ...
    setSavingExpense(true); // Start saving state
    setError(null); // Clear previous errors
    setSuccessSaving(false); // Clear previous success

    // Prepare updated data to send to the API
    const updatedExpenseRecordData = {
        ...expenseRecordData, // Spread the current state
        // Convert amount to a number before sending to the API
        amount: parseFloat(expenseRecordData.amount),
         // Ensure animalId is stored as a string (MongoDB ObjectId string) or null if none selected
        animalId: expenseRecordData.animalId || null,
         // Do NOT include the _id in the request body for PUT; it's in the URL
    };

    try {
        // --- API Call to update data ---
        const response = await fetch(`/api/expenses/${recordId}`, { // Use dynamic API route with recordId
            method: 'PUT', // Use PUT method for updates
            headers: {
                'Content-Type': 'application/json', // Tell the server we're sending JSON
            },
            body: JSON.stringify(updatedExpenseRecordData), // Send the updated expense data as a JSON string
        });

        if (!response.ok) {
            // If the response is not OK, throw an error
            const errorData = await response.json(); // Try to read error message from response body
            throw new Error(errorData.message || `Failed to update expense record with ID ${recordId}`);
        }

        // If the response is OK, the expense record was updated successfully
        const result = await response.json(); // Parse the JSON response (optional)
        console.log('Expense record updated successfully:', result);

        setSuccessSaving(true); // Set success state
        // Optional: Provide feedback to the user (e.g., a toast notification)
        // alert('Expense record updated successfully!'); // Using alert for simplicity

        // Navigate back to the view expenses page after saving
        ...
        setSavingExpense(false); // Stop saving state regardless of success or failure
    }
  };
  ...
  if (loadingExpense || loadingAnimals) {
    return <div className="text-center text-gray-600">Loading data...</div>;
  }

  if (error && !loadingExpense && !loadingAnimals) { // Only show error if not currently loading either expense or animals
  ...
  return (
    <div className="container mx-auto p-4 max-w-md">
      <h1 className="text-2xl font-bold mb-6 text-center">Edit Expense Record (ID: {recordId})</h1>

      {/* Loading, Error, and Success Messages for SAVING */}
      {savingExpense && <p className="text-center text-blue-600">Saving changes...</p>}
      {/* We handle fetch errors above, so this is primarily for save errors */}
      {error && !savingExpense && <p className="text-center text-red-600">Error: {error}</p>}
      {successSaving && !savingExpense && <p className="text-center text-green-600">Changes saved successfully!</p>}


      {/* The Edit Form - Conditionally render the form */}
      {/* Render the form if not loading expense data and no general error */}
      {!loadingExpense && !error && (
           <form className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" onSubmit={handleSubmit}>
             ...
             <div className="mb-4">
               ...
               <input
                 ...
                 name="date"
                 value={expenseRecordData.date} // Connect to state
             ...
             {/* Description */}
              <div className="mb-4">
                ...
                <input
                  ...
                  name="description"
                  value={expenseRecordData.description} // Connect to state
                  onChange={handleInputChange} // Use the generic handler
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  name="amount"
                  value={expenseRecordData.amount} // Connect to state
                  onChange={handleInputChange} // Use the generic handler
              ...
              <div className="mb-4">
                ...
                <select
                  ...
                  id="category"
                  name="category"
                  value={expenseRecordData.category} // Connect to state
                  onChange={handleInputChange} // Use the generic handler
               ...
               <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="animalSelect">
                 ...
                 {loadingAnimals ? (
                   ...
                   <select
                     ...
                     id="animalSelect"
                     name="animalId"
                     value={expenseRecordData.animalId} // Connect to state
                     onChange={handleInputChange} // Use the generic handler
                   >
             ...
             <div className="mb-6">
               ...
               <textarea
                 ...
                 name="notes"
                 rows="3"
                 value={expenseRecordData.notes} // Connect to state
                 onChange={handleInputChange} // Use the generic handler
               ></textarea>
             </div>
             ...
             <div className="flex items-center justify-between">
               <button
                 ...
                 type="submit"
                 disabled={savingExpense} // Disable button while saving
               >
                 {savingExpense ? 'Saving...' : 'Save Changes'} {/* Button text changes */}
               </button>
                {/* Optional: Cancel button */}
                <button
                  type="button" // Important: use type="button" so it doesn't submit the form
                  className="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800"
                  onClick={() => router.push('/expenses')} // Navigate back to view page
                >
                  Cancel
                </button>
             </div>
      ...
      {/* Message if record not found after loading or if there was a fetch error but form data is empty */}
       {/* We show this if not loading, no error, but expenseRecordData is still in its initial empty state */}
       {!loadingExpense && !error && expenseRecordData.description === '' && (
           <p className="text-center text-red-600">Expense record with ID {recordId} not found or could not be loaded.</p>
       )}
    </div>
  );
}
